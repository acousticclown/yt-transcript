...
if (!process.env.JWT_SECRET || process.env.JWT_SECRET === "notely-secret-key-change-in-production") {
  logger.warn("⚠️  WARNING: JWT_SECRET is not set or using default value. This is insecure in production!");
}

// Validate ENCRYPTION_KEY length (must be exactly 32 bytes for AES-256-CBC)
if (ENCRYPTION_KEY.length !== 32) {
  logger.warn(`⚠️  WARNING: ENCRYPTION_KEY length is ${ENCRYPTION_KEY.length}, should be 32 bytes`);
}
...
router.post("/login", async (req: Request, res: Response) => {
  try {
    logger.debug("🔐 Login attempt:", { email: req.body?.email, hasPassword: !!req.body?.password });
    
    const { email, password } = req.body;

    if (!email || !password) {
      logger.debug("❌ Login validation failed: missing email or password");
    ...
    if (!JWT_SECRET || JWT_SECRET === "notely-secret-key-change-in-production") {
      logger.error("❌ JWT_SECRET is missing or using default value!");
    }
...
router.post("/signup", async (req: Request, res: Response) => {
  try {
    ...
    logger.log("✅ User signed up:", user.email);
    res.status(201).json({
      token,
      user: {
        id: user.id,
  ...
  try {
    ...
    if (apiKey) {
      try {
        ...
        logger.warn("⚠️ Non-auth error, saving key anyway");
      }
    }
    ...
    logger.log("✅ Gemini API key updated for user:", decoded.userId);
    res.json({ success: true, hasGeminiKey: !!apiKey });
  } catch (error) {
    logger.error("❌ Gemini key update error:", error);
...
router.delete("/gemini-key", async (req: Request, res: Response) => {
  try {
    ...
    logger.log("✅ Gemini API key removed for user:", decoded.userId);
    res.json({ success: true });
  } catch (error) {
    logger.error("❌ Gemini key delete error:", error);
